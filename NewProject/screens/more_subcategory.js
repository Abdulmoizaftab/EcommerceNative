import {
  ...
  const More_subcategory = ({navigation, route}) => {
    const [products, setProducts] = useState([]);
    const {data} = route.params;
    const [limit, setlimit] = useState(6);
    const [isLoading, setIsloading] = useState(true);
    const [hier_id, setHier_id] = useState();
    ...
    const getdata = async () => {
        if(!hier_id){
        const hierarchy_id=data.hierarchy_id
        setIsloading(true)
        setProducts([]);
        await axios.post(
    ...
    return (
      ...
      />
          ...
          <View style={Style.head_text_view}>
            <Text style={Style.head_text}>{data.category_name.charAt(0).toUpperCase() + data.category_name.slice(1)}</Text>
          ...
          <FlatList
            ListHeaderComponent={
              <View style={{backgroundColor:"white"}}>
                <Tabs hierarchy_id={data.hierarchy_id} navigation={navigation} setHier_id={setHier_id} dont_navigate={true} />
  ...
  export default More_subcategory;
