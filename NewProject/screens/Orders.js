...
const Orders = ({navigation}) => {
  ...
  const getData = async () => {
    try {
      if(currentUser){
        setLogin(true)
        const res = await axios.post(`http://192.168.1.5:5000/sql/getOrderDetails/${limit}`,{user_id:currentUser.user[0].user_id},{
          headers: {
        ...
        if(e == "AxiosError: Request failed with status code 401"){
            Alert.alert(
                ...
                [
              {
                ...
                onPress: async () => {
                    try {
                        const res= await axios.post('http://192.168.1.5:5000/sql/session',{user_id:currentUser.user[0].user_id},{
                            headers: {
                                'Authorization': `Bearer ${currentUser.token}` 
                            }
                        })
