...
const SeeAllProducts = () => {
  ...
  const getdata = async () => {
    setIsloading(true)
    await fetch(`http://192.168.1.18:5000/sql/all/${limit}`)
  ...
  const handleFilterPrice = async (asc_desc)=>{
    ...
    await fetch(`http://192.168.1.18:5000/sql/filterAllByPrice/${asc_desc}/${limit}`)
    .then((response) => response.json())
    .then((json) => { setProducts(json) })
    .then(check=>  setIsloading(false))
    .catch((error) => console.error(error))
  ...
  const handleFilterRating = async (asc_desc)=>{
    setIsloading(true)
    //setIsRefreshing(true)
    await fetch(`http://192.168.1.18:5000/sql/filterAllByRating/${asc_desc}/${limit}`)
    .then((response) => response.json())
    .then((json) => { setProducts(json) })
    .then(check=>  setIsloading(false))
    .catch((error) => console.error(error))
